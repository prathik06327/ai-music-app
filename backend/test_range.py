...
RANGE_TEST_DIR = OUTPUTS_DIR / "range_test"
...
def _build_test_audio_paths() -> tuple[str, str]:
    ...
    RANGE_TEST_DIR.mkdir(parents=True, exist_ok=True)
    reference_path = RANGE_TEST_DIR / "reference.wav"
    user_path = RANGE_TEST_DIR / "user.wav"
...
def main():
    """
    Test script for vocal range analysis.
    """
    logging.basicConfig(level=logging.INFO, format="%(message)s")

    print("-" * 40)
    print("Vocal Range Test")
    print("-" * 40)

    reference_audio, user_audio = _build_test_audio_paths()

    try:
        result = analyze_vocal_range(reference_audio, user_audio)

        print(f"Reference Range: {result['reference_range']:.2f}")
        print(f"User Range: {result['user_range']:.2f}")
        print(f"Range Score: {result['range_score']:.2f}")
    except FileNotFoundError as exc:
        print(f"Error: {exc}")
    except Exception as exc:
        print(f"Error executing range analysis:\n{exc}")
    finally:
        print("-" * 40)
